...
export const useWordMatchGameStore = create(
    persist<WordMatchGameState>(
        (set,get)=>({
            ...
            addWordToArrangement: (word)=> set((state)=>({
                arrangedWords: [...state.arrangedWords, word],
            })),

            removeWordFromArrangement: (index) => set((state)=>({
                arrangedWords: state.arrangedWords.filter((_,item)=> item !== index),
            })),
        ...
        checkAnswer:()=>{
            ...
            const currentChallenge = challenges.find(c => c.id === currentLevel)
             ...
             const isWordCorrect = JSON.stringify(arrangedWords) === JSON.stringify(currentChallenge.correctOrder)
