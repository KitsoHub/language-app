...
export default function MascotAlert({ isCorrect, visible }: MascotAlertProps) {
    ...
    const animatedStyle = Platform.OS !== 'web' ? useAnimatedStyle(() => {
        return {
          transform: [
  ...
  if (Platform.OS === 'web') {
    return visible ? (
      <View style={styles.container}>
        <View style={[styles.bubble, isCorrect ? styles.correctBubble : styles.incorrectBubble]}>
          <Text style={styles.bubbleText}>
            {isCorrect ? "Great job! 🎉" : "Try again! 🤔"}
          </Text>
        </View>
        <Image
          source={mascotImageUrl}
