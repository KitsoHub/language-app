...
export default function AppChallenges() {
  ...
  const handleGameReset = () => {resetGame()}
 ...
 const totalChallenges = currentGame.challenges.filter(c => c.languageId === currentLanguage?.id).length;
  useEffect(() => {
    ...
    console.log("Is Last exercise: ", isLastTask);
		...
		switch (currentGame.type) {
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 40, marginBottom:40 }}>
              <Button title="Reset" style={styles.resetButton} onPress={handleGameReset} />
              <Button title="Check" style={styles.checkButton} onPress={handleWordCheck} disabled={isWordCheckDisabled} />
            </View>
					</SafeAreaView>
				);
